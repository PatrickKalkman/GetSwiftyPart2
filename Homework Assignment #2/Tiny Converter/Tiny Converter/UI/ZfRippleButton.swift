...
open class ZFRippleButton: UIButton {
    ...
    fileprivate func setupRippleView() {
        ...
        let xPosition: CGFloat = (bounds.width / 2) - (size / 2)
        let yPosition: CGFloat = (bounds.height / 2) - (size / 2)
        let corner: CGFloat = size / 2

        rippleView.backgroundColor = rippleColor
        rippleView.frame = CGRect(x: xPosition, y: yPosition, width: size, height: size)
